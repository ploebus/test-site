...
<div class="row">
	...
	<div class='col-md-5'>
		<h5 id="downloadData"><a href="#" ><i class="glyphicon glyphicon-save"></i> Click here to download child file</a></h5>
		<h5 id="downloadActivityData"><a href="#" ><i class="glyphicon glyphicon-save"></i> Click here to download activity file</a></h5>
	</div>
...
<script type="text/javascript">
    ...
    $('#childTable').DataTable();
		...
		$('#downloadData').click(function(event){
			event.preventDefault();
			console.log('Success')
			$.getJSON('download_child_file',function(data){
				  var items = data;
		...
		$('#downloadActivityData').click(function(event){
			event.preventDefault();
			console.log('Success')
			$.getJSON('download_activity_file',function(data){
				  var items = data;
        	const replacer = (key, value) => value === null ? '' : value; // specify how you want to handle null values here
        	const header = Object.keys(items[0]);
        	let csv = items.map(row => header.map(fieldName => JSON.stringify(row[fieldName], replacer)).join(','));
        	...
        	var url = URL.createObjectURL(blob);
        	downloadLink.href = url;
        	downloadLink.download = "activity.csv";  //Name the file here
        	document.body.appendChild(downloadLink);
        	downloadLink.click();
        	document.body.removeChild(downloadLink);
			});
